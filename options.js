...
module.exports = yargs
  .example(
    '$0 --token XYZ --target src/l10n/{sheet}/{locale}.js --moduleType AMD --sheets my_sheet,other_sheet',
  ...
  .option('auth', {
    ...
    demandOption: false,
    describe:
      'Path to auth.json file that has "private_key" and "client_email" fields. This file can be downloaded from Google Console (see docs).',
    type: 'string',
    coerce: (value) => {
    ...
    coerce: (pathToCustomOptions) => {
      ...
      try {
        ...
        if (pathToCustomOptions !== TRIGGER_DEFAULT_VALUE_FOR_PATH_OPTION) {
          console.log(
            `Error! Custom options file "${userCustomFnPath}" is not readable, using default options.`
          )
        }
